...
`KEY=`처럼 값이 비어 있는 항목도 `New value`로 채워 넣을 수 있습니다.

새 항목을 추가하려면 상단의 `New item` 영역에 `Key`와 `Value`를 입력하고 `Add`를 누릅니다. dot-env 형식 파일에는 `KEY=value` 줄이 파일 끝에 추가되고, JSON 파일에는 루트 객체에 문자열 속성이 추가됩니다.

기존 원본 값은 입력칸에 미리 채워 넣지 않습니다. 기존 값을 보여 주지 않는 대신, 새 값으로 교체하는 방식입니다.

현재 값을 확인해야 할 때는 각 항목의 눈 버튼을 누르면 됩니다. 눈 버튼을 누르기 전까지 Secure Editor 화면에는 원본 값이 전달되지 않습니다. 눈 버튼을 누르면 해당 값이 화면에 실제로 표시되므로 방송이나 화면 공유 중에는 사용하지 않는 것이 안전합니다.
...
- Secure Editor의 눈 버튼은 사용자가 직접 누른 값만 화면에 표시합니다. 표시된 값은 화면 공유에 노출될 수 있습니다.
